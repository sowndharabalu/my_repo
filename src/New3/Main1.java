...
class University{
    ...
    public void addStudent(Student student)throws EntrollmentException{
        Optional.of(student).filter(s->!students.containsKey(s.id())).orElseThrow(()->new EntrollmentException("Student already exists"));
        students.put(student.id(),student);
    }
    public void addCourse(Course course)throws EntrollmentException{
        Optional.of(course).filter(c->!courses.containsKey(c.code())).orElseThrow(()->new EntrollmentException("Course already exists"));
    ...
    public List<Course> getCoursesForStudent(String studentId) {
        return Optional.ofNullable(students.get(studentId)).map(s->s.entrolledCourses().stream().map(courses::get).filter(Objects::nonNull).toList()).orElseGet(List::of);
    }
    ...
    public double getAverageAgeInDepartment(Department department){
        var codes= courses.values().stream().filter(c->c.department()==department).map(Course::code).toList();
        return students.values().stream().filter(s->s.entrolledCourses().stream().anyMatch(codes::contains)).mapToInt(Student::age).average().orElse(0.0);
    }
    ...
    public Optional<Course> getMostPopularCourse(){
        return courses.values().stream().max(Comparator.comparingInt(c->getStudentsInCourse(c.code()).size()));
    }
}
...
public class Main1 {
    public static void main(String[] args) throws EntrollmentException{
        ...
        System.out.println("Students in CS101: " + uni.getStudentsInCourse("CS101"));
        System.out.println("Alice's courses: " + uni.getCoursesForStudent("S1"));
        System.out.println("Alice's credits: " + uni.getTotalCredits("S1"));
        System.out.println("Avg age in CS: " + uni.getAverageAgeInDepartment(Department.CS));
        System.out.println("Students with 4+ credits: " + uni.findStudentsWithMinCredits(4));
        System.out.println("Most popular: " + uni.getMostPopularCourse());
    }
}
